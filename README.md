...
또한 예외 핸들링 및 트랜잭션 관리에 어려움을 겪었고, 열심히 리팩토링 해봤지만 여전히 불필요한 부분이 보이는 것 같아 추후 더 개선된 코드로 수정하고 싶다.<br>
마지막으로 아무것도 모르는 상태로 예외가 발생하면 해결하기까지 너무 오래걸리기 때문에 발생한 예외와 해결 방안을 기록하여 나중에 비슷한 예외가 발생해도 해결 시간을 단축할 수 있도록 노력해야겠다.
